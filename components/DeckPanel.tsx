...
export default function DeckPanel({
  ...
  // ================= SAVE (WITH AUTO INCREMENT TITLE) =================
  ...
  return (
    <div className="w-full h-full p-4 bg-gradient-to-b from-gray-900 to-black text-white overflow-y-auto">
      ...
      {orderedTypes.map(type => {
        ...
        return (
          <div key={type} className="mb-6">
            <h3 className="text-lg font-bold mb-2">{type === "Cookie" ? "Cookies" : type}</h3>
            <div className="grid grid-cols-3 gap-3">
              {cardsInType.map(card => (
                <div key={card.id} className="relative cursor-pointer" onClick={() => removeOne(card.id, false)}>
                  <img src={`/cards/${card.id}.jpg`} className="w-full rounded-md shadow-lg" />
                  <div className="absolute bottom-2 left-2 bg-black/70 px-2 py-1 rounded text-sm font-bold">x{card.qty}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {Object.keys(extraDeck).length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-bold mb-2">Extra</h3>
          <div className="grid grid-cols-3 gap-3">
            {Object.entries(extraDeck).map(([id, qty]) => (
              <div key={id} className="relative cursor-pointer" onClick={() => removeOne(id, true)}>
                <img src={`/cards/${id}.jpg`} className="w-full rounded-md shadow-lg" />
                <div className="absolute bottom-2 left-2 bg-black/70 px-2 py-1 rounded text-sm font-bold">x{qty}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg w-[500px]">
            ...
            <textarea
              ...
              placeholder="4 ST6-002-Fire Spirit Cookie"
              value={importText}
              onChange={(e) => setImportText(e.target.value)}
            />
            <div className="flex justify-end gap-3 mt-4">
              <button className="bg-blue-600 px-4 py-2 rounded-md" onClick={() => parseImport(false)}>Import and add</button>
              <button className="bg-green-600 px-4 py-2 rounded-md" onClick={() => parseImport(true)}>Import and replace</button>
              <button className="bg-gray-600 px-4 py-2 rounded-md" onClick={() => setShowImport(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
